 ...
 * we have to deal it separately
 */
int capture_send_arp_remote(const struct sk_buff *skb, struct capture_info *cache, int output)
{
    int ret;
    unsigned int headlen = 0;        
    ...
    */
    ...
    skb_reset_mac_header(skb_cp);    
    ethh = eth_hdr(skb_cp);
    memcpy(ethh->h_source, cache->addr.local_mac, ETH_ALEN);
    memcpy(ethh->h_dest, cache->addr.remote_mac, ETH_ALEN);
    ethh->h_proto = htons(ETH_P_IP);  

    skb_cp->dev = cache->dev;

    ret = dev_queue_xmit(skb_cp);
    if (unlikely(ret != NET_XMIT_SUCCESS && ret != NET_XMIT_CN)) 
        log_debug("dev queue xmit packet fail. \r\n");
...
//发往外部服务器
int capture_send_remote(const struct sk_buff *skb, struct capture_info *cache, int isrtp, int output)
{
    unsigned seq;
    int ret, hh_len, hh_alen;
...
EXPORT_SYMBOL(capture_send_remote);
...
void capture_match_and_send(const struct sk_buff *skb,unsigned short port,int isrtp,int output)
{
    ...
    unsigned int captured_id[USER_MAX];
    int index, count, captured_flag;
    ...
    if(capture_mgr_head.mgr != NULL)
    {
        ...
        memset(&captured_id, 0 , sizeof(unsigned int));
        captured_flag = 0;
        index = 0;
        count = 0;
             ...
             if(mgr->ports.min <= port && port <= mgr->ports.max)         
             {
                ...
                while(ctl != NULL)
                {
                    ...
                    for(index = 0; index < count; index++) {
                        if (captured_id[index] == ctl->cache.taskid) {
                            captured_flag = 1;
                        }
                    }
                    if (!captured_flag) {
                        ...
                        {    
                            if(0 == is_capture_addr_match(iph,&(ctl->cache)))
                            {
                                ...
                                {
                                    // 记录该用户ID
                                    captured_id[count] = ctl->cache.taskid;
                                    count ++;
                                    
                                    // 远端服务器 or 本地服务器
                                    if (ctl->cache.localflag)
                                        capture_send_remote(skb, &(ctl->cache), isrtp, output);
                                    else                                        
        ...
        while(ctl != NULL)
        {
            ...
            if(ctl->cache.protocol & CAPTURE_ARP)
            {
                ...
                if(flag == 0)
                {
                    ...
                    /* copy and send */
                    if (ctl->cache.localflag)
                        capture_send_arp_remote(skb, &(ctl->cache), output);
                    else    
                ...
                if(0 == is_capture_addr_match(iph,&(ctl->cache)))
                {
                    if(0 == is_capture_localpkt_match(iph,&(ctl->cache)))
                    {
                        ...
                        if(flag == 0)
                        {
                            ...
                            /* copy and send */
                            if (ctl->cache.localflag)
                                capture_send_remote(skb, &(ctl->cache), 0, output);
                            else    
       ...
       while(mgr != NULL)
       {
            ...
            while(ctl != NULL)
            {
                ...
                if(ctl->cache.taskid == taskid)
                {
                    ...
                    log_debug("delete ctl block by taskid : %u",ctl->cache.taskid);
                    kfree(ctl);
                    ctl = NULL;
                    /* check next ctl */
                    if (ctl_prv != NULL)
                        ctl = ctl_prv->next_ctl;
                    else    
                        ctl = mgr->ctl;                                        
                }
    ...
    while(1)
    {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (!localflag) {
                            ...
                            mgr->ctl->cache.addr.local_ip.ip = in_aton("1.1.1.1");
                            //mgr->ctl->cache.addr.local_ip.ip = addr->local_ip.ip;                            
                        }
                        else {
                            // 远端抓包
                            mgr->ctl->cache.dev = dev_get_by_name(&init_net, addr->netface);                        
                            if (!mgr->ctl->cache.dev) {
                                log_info("get dev fail \r\n");
                                kfree(capture_mgr_head.mgr);
                                kfree(mgr->ctl);
                                capture_mgr_head.mgr = NULL;
                                mgr = NULL;
                            ...
                            mgr->ctl->cache.addr.local_port = addr->local_port;
                            mgr->ctl->cache.addr.local_ip.ip = addr->local_ip.ip;
                        }
                        ...
                        memcpy(mgr->ctl->cache.addr.local_mac, addr->local_mac, ETH_ALEN);
                        memcpy(mgr->ctl->cache.addr.remote_mac, addr->remote_mac, ETH_ALEN);   
                        memcpy(mgr->ctl->cache.addr.netface, addr->netface, 32);
        ...
        else
        {
            ...
            if((mgr->ports.min == filter->min) && (mgr->ports.max == filter->max))
            {
                ...
                while(ctl != NULL)
                {
                    /* 允许同一个端口范围有同一个用户的多个过滤条件
                    if(ctl->cache.taskid == taskid)
                    {
                        log_debug("task already exist, But could be same user \r\n");
                        goto end;
                    }
                    else
                    */
                ...
                {           
                    ...
                    if (!localflag) {
                        ...
                        ctl->cache.addr.local_ip.ip = in_aton("1.1.1.1");
                            //mgr->ctl->cache.addr.local_ip.ip = addr->local_ip.ip;                            
                    }
                    else {
                        ctl->cache.addr.local_port = addr->local_port;
                    ...
                    memcpy(ctl->cache.addr.local_mac, addr->local_mac, ETH_ALEN);
                    memcpy(ctl->cache.addr.remote_mac, addr->remote_mac, ETH_ALEN);   
                    memcpy(ctl->cache.addr.netface, addr->netface, 32);
                    ctl->cache.dev = dev_get_by_name(&init_net, addr->netface);
                    if (!ctl->cache.dev) {
                        log_info("get dev fail \r\n");
                        kfree(ctl);
                        goto end;                                                    
...
next:
    ...
    else
    {
        ...
        while(task != NULL)
        {
            if(task->taskid == taskid)
            {
                log_debug("task already exist, Maybe just the same task \r\n");
                goto end;
            }
            else
            {
...
static int capture_netlink_rcv_msg(struct sk_buff *skb, struct nlmsghdr *nlh)
{
    ...
    switch(nlh->nlmsg_type){
        ...
        case CAPTURE_ADD :
            ...
            //if remote server unreachable, then drop it 
            if (task->localflag == 1) {
                if ((NMAC1(task->addr.remote_mac) == 0) && (NMAC2(task->addr.remote_mac) == 0))
                    if ((NMAC3(task->addr.remote_mac) == 0) && (NMAC4(task->addr.remote_mac) == 0))
                        if ((NMAC5(task->addr.remote_mac) == 0) && (NMAC6(task->addr.remote_mac) == 0)) {
                            log_info("remote server mac is zero, capture fail \r\n");
                            return -1;
                        }
            }
            ...
            while(index < filter_count)
            {
                  ...
                  if(task->filter[index].max > 65535)
                  {
                      task->filter[index].max = 65535;
                  }
                  ...
                  log_info("add task: \n"
                        ...
                        "daddr        :%d:%d:%d:%d \n"
                        "====================\r\n",
                    task->taskid,task->localflag,task->timerval,task->islocalpkt,ntohs(task->addr.local_port),ntohs(task->addr.remote_port),
                    CIP1(task->addr.local_ip),CIP2(task->addr.local_ip),CIP3(task->addr.local_ip),CIP4(task->addr.local_ip),ntohs(task->addr.local_port),
                    ...
                    NMACQUAD(task->addr.local_mac), NMACQUAD(task->addr.remote_mac), task->addr.netface,
