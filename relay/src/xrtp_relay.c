...
#define RTP_DSP_CHAN_DIR_ORIGINAL   1       /* original 方向 */
#define RTP_DSP_CHAN_DIR_REPLY      2       /* reply 方向 */

static unsigned int g_rtp_hz_ticks = 1000;
static unsigned int g_master_solt_id = 0;   /* 当前主控板槽位号 */
...
/* 当某一连接流量超过一定得阀值时(缺省为30KB/sec,最大711编码10KB/sec)，告警处理 */
...
#define RTP_JITTER_BUFFER           1   /* 开启jitter buffer功能 */
//#define RTP_JITTER_KTHREAD          1   /* 使用内核线程发送队列包 */
#define RTP_JITTER_WORK_QUEUE       1   /* 使用工作队列实现发送队列包 */
#define RTP_JITTER_BUFFER_HRTIMER   1   /* 使用高精度定时器 */

#define RTP_JITTER_BUFFER_PKTS      64  /* 每个通道最大缓存包个数 */
...
struct rtp_skb_cache_ctrl {
    struct list_head cache_head;    /* 当前缓存报文链表 */
    struct list_head list_free;     /* 空闲缓存链表 */
    unsigned int input_cnt;         /* 队列接收报文总数 */
    unsigned int output_cnt;        /* 队列发送报文总数 */
    unsigned int used_cnt;          /* 队列当前缓存报文数 */
    unsigned int packet_time;       /* 通道打包时长 */
    unsigned int used_max;          /* 某一时候最大缓存报文数 */
    unsigned int overflow;          /* 队列报文溢出数 */
    spinlock_t lock;
};
...
///-
    ...
    /* 处理上层下发mac地址为0时，使用媒体刷新机制更新mac地址表项 */
...
static void rtp_netlink_tbl_delete(unsigned int connid)
{
    ...
    if (ctrl_tbl->cache != NULL)
    {
        ...
        /* 连接删除，上报统计信息到用户态管理进程，目前只有安全管理进程关注该通知 */
        ...
        if (cache->aconn.media_lock == 1)
        {
            ...
            if (rtp_media_lock_match_v4(ethh, iph, udph, &cache->aconn))
            {
                ...
                if (rtp_media_lock_match_v4(ethh, iph, udph, &cache->bconn))
                {
                    ...
                    ///-
                        /*去除提前设置rtcp的媒体信息，rtcp由rtcp流程自己刷新 --liwen*/
                        #if 0
                        rtp_write_lock(&g_rtp_ctrl_tbl[connid + 1].lock);
                        if(g_rtp_ctrl_tbl[connid + 1].cache != NULL)
                        {
                        g_rtp_ctrl_tbl[connid + 1].cache->aconn.remote_ip.ip = iph->saddr;
                        //g_rtp_ctrl_tbl[connid + 1].cache->aconn.remote_port = udph->source;
                        }
                        rtp_write_unlock(&g_rtp_ctrl_tbl[connid + 1].lock);
                        #endif
                        return &cache->bconn;
                    }
                    else
                    ...
                    ///-
                        ...
                        #if 0
                        rtp_write_lock(&g_rtp_ctrl_tbl[connid + 1].lock);
                        if(g_rtp_ctrl_tbl[connid + 1].cache)
                        {
...
static int rtp_relay_log_rfc2833_v4(struct iphdr *iph,struct udphdr *udph, struct rtphdr *rtph, struct rtp_media *fw_conn)
{
    ...
    if(rfc2833h != NULL)
    {
        LOG_INFO("rfc2833 event_id :%u,flag :%u,event_duration :%u, media_lock :%u,ip.saddr: %d:%d:%d:%d:%hu,ip.daddr: %d:%d:%d:%d:%hu \r\n",
...
static int rtp_relay_check_packet_v4(struct udphdr *udph, struct rtphdr *rtph, struct rtp_media *fw_conn)
{
    ...
    if (rtph->rtp_paytype != fw_conn->media_data.rtp.payload)
    {
        ...
        /* 不匹配payload type的情况下，判断是否是dtmf报文 */
            ...
            struct rtp_media *fw_conn)
{
    ...
    /*有些设备可能发送的端口号和接收端口号不一致，但ip不可能出现，否则改设备与其他的软phone也不能
      对接*/
...
/*
 * 截取RTP媒体报文处理, 物理上LAN或者WAN都执行相同处理
...
///-
    ...
    {
        /*
         * relay_rtp_ddos_detect(skb); 交由安全模块处理
         */
        return NF_ACCEPT;
    }
    udph = udp_hdr(skb);
    ...
    if (g_rtp_ctrl_tbl[dport].cache != NULL)
    {
        ...
        else
        {
            ...
            if(RM_MEDIA_IMAGE != fw_conn->media_type)
            {
                ...
                /* rtp报文合法性检测, rtp_check_packet合法返回值为0 */
        ...
        if (fw_conn->ip_type == PF_INET)
        {
            if (!rtp_relay_update_packet_v4(skb, iph, udph, fw_conn))
            {
                ...
                /*如果目的和本地某个接口的ip相同，报文是要在本设备上再转一次，通过目的端口号来判断是要在本板转发，
                  还是要发往另一个用户板，如果是本板转发，直接再调一次函数，不能通过网口的acl规则来将
                  本cpu的报文环回*/
                if(unlikely((g_rtp_ctrl_tbl[ntohs(udph->dest)].cache &&
                    (iph->daddr == g_rtp_ctrl_tbl[ntohs(udph->dest)].cache->aconn.local_ip.ip ||
                    iph->daddr == g_rtp_ctrl_tbl[ntohs(udph->dest)].cache->bconn.local_ip.ip))))
                {
                       ...
                       int (*okfn)(struct sk_buff *))
{
    ...
    ip = ip_hdr(skb);
    /* Netfilter LOCAL_IN钩子的报文不做ip头的判断 */
    #if 0
    if (!ip)
    {
        return NF_ACCEPT;
    ...
    if (fw_conn->ip_type == PF_INET)
    {
        ...
        if(unlikely((g_rtp_ctrl_tbl[ntohs(newudp->dest)].cache &&
            (newip->daddr == g_rtp_ctrl_tbl[ntohs(newudp->dest)].cache->aconn.local_ip.ip ||
                    newip->daddr == g_rtp_ctrl_tbl[ntohs(newudp->dest)].cache->bconn.local_ip.ip))))
        {
            skb_pull(skb, sizeof(struct ethhdr));
