...
void capture_match_and_send_icmp(const struct sk_buff *skb)
{
    int i = 0;
    int flag = 0;
    struct capture_mgr *mgr = NULL;
			       ...
			       int (*okfn)(struct sk_buff *))
{
    ...
    {
        capture_match_and_send_icmp(skb);
        return NF_ACCEPT;
    }
...
static void capture_netlink_delete(unsigned int taskid)
{
    ...
    LOG_DEBUG("delete taskid:%hu done \r\n",taskid);
    return ;
}
...
void capture_timer_handler(unsigned long taskid)
{
    ...
    LOG_DEBUG("reach time limit, delete task:%hu \r\n",id);
    return ;
}
...
static void capture_add_timer(unsigned int taskid,unsigned int timeval)
{
    ...
    for(;i<USER_MAX;i++)
    {
        if(g_capture_timer[i].taskid == taskid)
        {
            LOG_WARN("task:%u timer already exist \r\n",taskid);
            return ;
        }        
    }
    for(i=0;i<USER_MAX;i++)
 ...
 */
static void capture_netlink_add(unsigned int taskid, unsigned int localflag,struct capture_addr *addr, struct capture_filter_nl *filter )
{
    struct capture_mgr *mgr = NULL;
    struct capture_mgr *mgr_prv = NULL;
    struct capture_ctl *ctl = NULL;
...
static int capture_netlink_rcv_msg(struct sk_buff *skb, struct nlmsghdr *nlh)
{
    ...
    switch(nlh->nlmsg_type){
        ...
        case CAPTURE_ADD :
            ...
            filter_count = (nlh->nlmsg_len - NLMSG_HDRLEN - sizeof(struct capture_addr) - sizeof(int)*3)/filter_len;
            LOG_DEBUG("receive %d filter \r\n",filter_count);
            /*
            ...
            while(index < filter_count)
            {
                  ...
                  (void)capture_netlink_add(task->taskid,task->localflag,&task->addr,&task->filter[index]);    
                  ...
                  LOG_DEBUG("add task: \n"
                        ...
                        "daddr        :%d:%d:%d:%d \n"
                        "====================\r\n",
                    task->taskid,task->localflag,task->timerval,task->addr.local_port,ntohs(task->addr.remote_port),
