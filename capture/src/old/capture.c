...
static int capture_debug = 1;
struct capture_mtu g_mtu[MTU_MAX];
unsigned int mtu = 0;
...
static int capture_netlink_rcv_msg(struct sk_buff *skb, struct nlmsghdr *nlh)
{
    ...
    switch(nlh->nlmsg_type){
        ...
        case CAPTURE_ADD :
            ...
            /*
             *  check if taskid already exsit in tasklist,
             *  if not then add;
             */                
            capture_read_lock(&capture_mgr_head.lock);
...
{	    
    memset(&g_mtu,0,sizeof(struct capture_mtu)*MTU_MAX);
    /* init mgr list head */
