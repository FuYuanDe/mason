 ...
 * 修改UDP报文的校验和并发送出�?
 */
int capture_sendto_server(struct sk_buff *skb, unsigned int taskid)
{
    unsigned int mtu = 0;
    struct iphdr *iph;
    struct udphdr *udph;
    ...
    if(mtu == 0)
    {
        ...
        {
            iph->frag_off = 0;   /* ip_fragment will set offset automaticly */           
        }
        else
        {
    ...
    iph->check = 0;
    skb->csum = 0;
	skb->csum = csum_partial(skb_transport_header(skb), htons(udph->len), 0);    
	udph->check = csum_tcpudp_magic(iph->saddr, iph->daddr, htons(udph->len), IPPROTO_UDP, skb->csum);	
    ...
    skb_dst_set(skb, (struct dst_entry *)skb->_skb_refdst);  
 ...
 *  是的话返回true，否则false
*/
int capture_check_remote_server(struct iphdr *iph)
{
        ...
        {
             {
                ...
                while(ctl != NULL)
                {
                    if(ctl->cache.addr.local_ip.ip == iph->saddr)
                    {    
                        if(ctl->cache.localflag)
                        {
                            iph->daddr = ctl->cache.addr.remote_ip.ip;
                            taskid  =ctl->cache.taskid;
			       ...
			       int (*okfn)(struct sk_buff *))
{
    ...
    if(sport == 0)
    {
        if(iph->protocol != IPPROTO_UDP)
            return NF_ACCEPT;
        /* check if we have to send to remote server */    
        taskid = capture_check_remote_server(iph);
        if(taskid)
        {
            capture_sendto_server(skb, taskid);
            return NF_STOLEN;
        }        
        return NF_ACCEPT;
    }
